...
InitInstructions()
{
  const RectangleType     rect  = {{0,0},{142,357}};
  ...
  // draw the help
...

    {
      ...
      // general text
      StrCopy(str, "Donkie Kung has captured a girl!");
      x = (rect.extent.x - FntCharsWidth(str, StrLen(str))) >> 1;
      WinDrawChars(str, StrLen(str), x, y); y += FntLineHeight();
      ...
      x = 4;
      StrCopy(str,
"Maryo is on a mission to save her and must move through an industrial \
landscape avoiding tumbling barrels and steel bars to grab keys that \
hold the platform Donkie Kung is standing on."); 
      ptrStr = str;
      ...
      // add space (little)
      y += 138 + (FntLineHeight() >> 1);

      // general text
      x = 4;
      StrCopy(str,
"Using the stylus or keys, move Maryo as shown in the diagram above. Start \
the crane using the switch and jump for the keys by jumping at the edge.");
      ptrStr = str;
      while (StrLen(ptrStr) != 0) {
        UInt8 count = FntWordWrap(ptrStr, rect.extent.x-x);
