         ...
         switch (event->data.menu.itemID)
         {
           ...
           case mainMenuItemShowHighA:
                ...
                if CHECK_SIGNATURE(globals->prefs) {
                  ...
                  globals->prefs->game.dkung.gameType = GAME_A;
           ...
           case mainMenuItemShowHighB:
                ...
                if CHECK_SIGNATURE(globals->prefs) {
                  ...
                  globals->prefs->game.dkung.gameType = GAME_B;
                  ApplicationDisplayDialog(highForm);
                }
                else {
                  SndPlaySystemSound(sndError);
...
gameFormEventHandler(EventType *event)
{
  ...
  switch (event->eType) 
  {
    ...
    case penMoveEvent:
         ...
         if (globals->prefs->game.gamePlaying)
         {
           ...
           if ((event->screenX > minx) && (event->screenX < maxx) &&
               (event->screenY > miny) && (event->screenY < maxy))
           {
         ...
         switch (event->data.menu.itemID)
         {
           ...
           case gameMenuItemReset:
                GameReset(globals->prefs, 
                          globals->prefs->game.dkung.gameType);
    ...
    case frmCloseEvent:
         ...
         if (globals->prefs->handera.device)
           VgaSetScreenMode(globals->prefs->handera.scrMode, 
...
InitApplication()
{
  ...
  if (globals->prefs->game.gamePlaying) {
    ...
    globals->prefs->game.dkung.displayChanged = true;
    FrmGotoForm(gameForm);
  }
  else {
    FrmGotoForm(mainForm);
...
ApplicationDisplayDialog(UInt16 formID)
{
  ...
  gameActive = (FrmGetActiveFormID() == gameForm);

  // force handera into 160 -> 240 stretch and 1to1 for bitmaps
  if ((globals->prefs->handera.device) && (gameActive))
    VgaSetScreenMode(globals->prefs->handera.scrMode, 
                     globals->prefs->handera.scrRotate);
